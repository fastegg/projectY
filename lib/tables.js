...
function initClientDB(ctx, cb) {
  ...
  function appendData(tableName, findParams, mask, next) {
  ...
  for (var table in gTables) {
    switch (gTables[table]) {
      case TABLE_TYPES.GLOBAL:
        series.push(getClientData, table, null, null);
        break;
      case TABLE_TYPES.ACCOUNT:
        if (!ctx.accountID) {
          break;
        }
        series.push(getClientData, table, {accountID: ctx.accountID}, null);
        break;
    }
  }
